...
public partial class CompanyService : BaseService, ICompanyService
{
	...
	protected readonly ICompanySettingService CompanySettingService;
    ...
    public CompanyService(IUow repo, IPublishEndpoint publishEndpoint, IMapper mapper, IConfiguration config, IBranchUserService branchUserService, IBranchService branchService, ICompanySettingService companySettingService, IPermissionService permissionService, IRoleService roleService, IUserCompanyService userCompanyService, IUserRoleService userRoleService, IUserService userService) : base(mapper, publishEndpoint, config)
    {
		...
		CompanySettingService = companySettingService ?? throw new ArgumentNullException(nameof(companySettingService));
    ...
    public async Task<Company> SaveAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        foreach (var child in entity.CompanySettings)
        {
            await CompanySettingService.SaveAsync(child, dataFilter, false);
        }
    ...
    public async Task<Company> UpdateAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        foreach (var child in entity.CompanySettings)
        {
            await CompanySettingService.UpdateAsync(child, dataFilter, false);
        }
    ...
    public async Task<bool> ArchiveAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        if (entity == null) throw new ArgumentNullException(nameof(entity));
        ...
        foreach (var child in existingEntity.CompanySettings)
        {
            await CompanySettingService.DeleteAsync(child, dataFilter, false);
        }
    ...
    private void ApplyValidationBl(Company entity)
    {
        try
        {
			...
			if (entity.CompanySettings == null) entity.CompanySettings = new List<CompanySetting>();
