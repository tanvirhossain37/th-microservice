...
public partial class CompanyService : BaseService, ICompanyService
{
    ...
    public CompanyService(IUow repo, IPublishEndpoint publishEndpoint, IMapper mapper, IBranchUserService branchUserService,
        IBranchService branchService, IPermissionService permissionService, IRoleService roleService, IUserRoleService userRoleService,
        IUserService userService) : base(mapper, publishEndpoint)
    {
    ...
    public async Task<Company> SaveAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        if (commit)
        {
            ...
            shadowEvent.SpaceId = UserResolver.SpaceId;
            shadowEvent.ClientId = entity.Id;
            shadowEvent.UserName = UserResolver.UserName;
            shadowEvent.ActivityName = ActivityNameEnum.Save;

            //publish
            await PublishEndpoint.Publish(shadowEvent);
    ...
    public async Task<Company> UpdateAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        var existingEntity =
            await Repo.CompanyRepo.SingleOrDefaultQueryableAsync(x => (x.SpaceId.Equals(entity.SpaceId)) && (x.Id.Equals(entity.Id)));
        ...
        if (commit)
        {
            ...
            var shadowEvent = new ShadowEvent();
            shadowEvent.SpaceId = UserResolver.SpaceId;
            shadowEvent.ClientId = existingEntity.Id;
            shadowEvent.UserName = UserResolver.UserName;
            shadowEvent.ActivityName = ActivityNameEnum.Update;
    ...
    public async Task<bool> SoftDeleteAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        if (commit)
        {
            ...
            shadowEvent.ClientId = existingEntity.Id;
            shadowEvent.UserName = UserResolver.UserName;
            shadowEvent.ActivityName = ActivityNameEnum.SoftDelete;
    ...
    public async Task<bool> DeleteAsync(Company entity, DataFilter dataFilter, bool commit = true)
    {
        ...
        if (commit)
        {
            ...
            shadowEvent.ClientId = existingEntity.Id;
            shadowEvent.UserName = UserResolver.UserName;
            shadowEvent.ActivityName = ActivityNameEnum.Delete;
    ...
    public async Task<IEnumerable<Company>> GetAsync(CompanyFilterModel filter, DataFilter dataFilter)
    {
        try
        {
            ...
            var pagedList = await Repo.CompanyRepo.GetFilterableAsync(predicates, includePredicates, filter.SortFilters, filter.PageIndex,
